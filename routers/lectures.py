...
def lecture_to_dict(lec):
    return {
        ...
        "view_count": lec.view_count or 0,
        "like_count": lec.like_count or 0,
        ...
        "category": {
            "id": lec.category.id,
            "name_en": lec.category.name_en,
            "name_ha": lec.category.name_ha,
            "slug": lec.category.slug,
        } if lec.category else None,
...
def get_lectures(
    ...
    q = db.query(models.Lecture).filter(
        (models.Lecture.is_published == True) | (models.Lecture.is_published == None)
    )
    if category_id:
...
def get_featured(db: Session = Depends(get_db)):
    lectures = (
        ...
        .filter(models.Lecture.is_featured == True)
        .order_by(desc(models.Lecture.created_at))
        .limit(10)
        .all()
    )
...
def get_trending(db: Session = Depends(get_db)):
    lectures = (
        db.query(models.Lecture)
        .filter(models.Lecture.is_trending == True)
        .order_by(desc(models.Lecture.view_count))
        .limit(10)
        .all()
    )
    return [lecture_to_dict(l) for l in lectures]
...
def get_lecture(lecture_id: int, db: Session = Depends(get_db)):
    ...
    lec.view_count = (lec.view_count or 0) + 1
    db.commit()
...
def create_lecture(
    ...
    lec = models.Lecture(
        ...
        description_ha=data.description_ha,
...
def delete_lecture(
    ...
    if not lec:
        raise HTTPException(status_code=404, detail="Not found")
    db.query(models.Video).filter(models.Video.lecture_id == lecture_id).delete()
    db.query(models.AudioFile).filter(models.AudioFile.lecture_id == lecture_id).delete()
    db.query(models.Favorite).filter(models.Favorite.lecture_id == lecture_id).delete()
    db.query(models.WatchHistory).filter(models.WatchHistory.lecture_id == lecture_id).delete()
    db.delete(lec)
    db.commit()
